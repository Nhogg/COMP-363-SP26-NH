...
When comparing my work to the solution, I recognized that  my approach relied on fixing problems as they appeared. The solution normalizes the inputs upfront by padding them to the same length before doing anything else.  Although my method worked and produced the correct result, the solution's approach is much cleaner and less prone to indexing errors.
...
When comparing my work to the solution, I realize I was more focused on efficiency. The solution uses a simple List to track visited nodes. Though the solution's method is correct, my approach is significantly faster for large graphs because Set lookups are instant ($O(1)$) while List lookups take longer as the list grows ($O(n)$).

In doing this assignment, I learned that choosing the right data structure is just as important as the algorithm itself. While the solution's logic was sound, using a Set makes the component counting significantly faster for large graphs.
